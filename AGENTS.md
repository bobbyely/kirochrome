...
| `http.ts` | JSON API (providers, checks, the agent's own sessions, search, export, the Files pane) + SPA, with the `Origin` allowlist |
...
| `startOptions.ts` | `cleanStart` — a form's start options (config values, opening message) kept to their proper types |
...
| `fs.ts` | `fs/read_text_file`, `fs/write_text_file` — for the agent, unsandboxed |
| `files.ts` | The Files pane's reader — for the browser, confined to the conversation's roots |
...
**`packages/web`** — the browser. It renders the event log and a live tail,
and holds no authoritative state (invariant 3).

| File | Job |
|---|---|
| `App.tsx` | Routes |
| `Chat.tsx` | **The other big one.** Transcript, scroll-following, every row renderer |
| `Composer.tsx` | The message box and everything that changes per keystroke — draft, images, completion, the queue, the provider and config pickers — kept out of `Chat` so typing does not re-render the transcript |
| `timeline.ts` | Folds events into typed rows |
| `useChat.ts` | The socket and resume-by-seq |
| `useReadyProviders.ts` | The checked-provider list every picker and form reads |
| `commands.ts`, `mentions.ts` | The pure halves of the `/` and `@` pickers |
| `Setup.tsx` | The ladder UI |
| `NewChat.tsx`, `StartPickers.tsx` | Starting a conversation; the model/mode/opening-message choice the forms share |
| `Schedules.tsx` | Schedule list, form and run history |
| `Rooms.tsx` | Room list, form and the room itself |
| `SidePane.tsx` | The drawer beside the transcript, with its two tabs |
| `changes.ts`, `ChangesPane.tsx` | Folds the log's reported diffs per file; the Changes tab |
| `files.ts`, `FilesPane.tsx` | A viewer per format; the Files tab's tree and viewers |
| `DiffView.tsx`, `diff.ts` | Diff rendering, shared by the tool cards, the Changes tab and the code viewer |
| `Sidebar`, `images`, `theme`, `useShortcuts`, `Markdown`, `KSpinner`, `api` | What their names say |
| `games/` | The Play panel: two word games, pure logic in `wordle.ts` and `anagram.ts`, and `words.ts` generated by `scripts/words.mjs` from the system dictionary and `scripts/common-words.txt` |
...
`session.test.mjs`, `queue.test.mjs` and `sessionFeatures.test.mjs` drive the
real mock agent over ACP, so they cover the behaviours that are easy to break:
delta coalescing, queue ordering, replay suppression on resume, defaults
surviving a withdrawn option, provider switching. Three files for time — each
has a 45-second cap, and the queue tests ride a ten-second turn.
