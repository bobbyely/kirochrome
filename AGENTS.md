...
`changes.ts` folds the log's reported diffs per file and `ChangesPane.tsx`
shows them, with `DiffView.tsx` shared with the tool cards; `files.ts` picks a
viewer per format and `FilesPane.tsx` is the tree and the viewers; `mentions.ts` is the `@` picker's pure half (where the mention is, what it
completes to, which chosen ones are sent); then
`Sidebar`, `NewChat`, `commands`, `diff`, `images`, `theme`, `useShortcuts`,
`Markdown`, `KSpinner`, `api`. `games/` is the Play panel: two word games,
pure logic in `wordle.ts` and `anagram.ts`, and `words.ts` generated by
`scripts/words.mjs` from the system dictionary and `scripts/common-words.txt`.
